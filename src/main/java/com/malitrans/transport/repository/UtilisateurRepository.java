...
public interface UtilisateurRepository extends JpaRepository<Utilisateur, Long> {
        ...
        Optional<Utilisateur> findByEmailOrPhone(String email, String phone);

        Optional<Utilisateur> findByUsernameOrEmailOrPhone(String username, String email, String phone);
         ...
         * using native SQL
         * Uses ILIKE for PostgreSQL case-insensitive search (avoids LOWER() on bytea
         * error)
         * 
         * @param role        Role as String (e.g., "CHAUFFEUR")
         * @param companyId   The company ID
         * @param status      Optional status filter as String (null to ignore)
         * @param searchQuery Optional search query for username, phone, or matricule
         *                    (null or empty to ignore)
        ...
        @Query(value = "SELECT u.* FROM utilisateur u WHERE " +
                        ...
                        "ORDER BY u.id DESC", countQuery = "SELECT COUNT(*) FROM utilisateur u WHERE " +
                                        "u.role = :role AND " +
                                        "u.company_id = :companyId AND " +
                                        "(:status IS NULL OR u.status = :status) AND " +
                                        "(:searchQuery IS NULL OR :searchQuery = '' OR " +
                                        "  u.username ILIKE '%' || :searchQuery || '%' OR " +
                                        "  u.phone ILIKE '%' || :searchQuery || '%' OR " +
                                        "  u.matricule ILIKE '%' || :searchQuery || '%')", nativeQuery = true)
